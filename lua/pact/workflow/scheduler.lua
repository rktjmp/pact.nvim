local result, _local_6_, enum, _local_7_, _local_8_ = nil, nil, nil, nil, nil
do
  local _5_ = vim
  local _4_ = string
  local _3_ = require("pact.lib.ruin.enum")
  local _2_ = require("pact.lib.ruin.type")
  local _1_ = require("pact.lib.ruin.result")
  result, _local_6_, enum, _local_7_, _local_8_ = _1_, _2_, _3_, _4_, _5_
end
local _local_9_ = _local_6_
local string_3f = _local_9_["string?"]
local thread_3f = _local_9_["thread?"]
local _local_10_ = _local_7_
local fmt = _local_10_["format"]
local _local_11_ = _local_8_
...
local function make_idle_loop(scheduler)
  local function _13_()
    ...
    local _let_14_ = require("pact.workflow")
    local run = _let_14_["run"]
    local _let_15_ = require("pact.lib.ruin.result")
    local ok_3f = _let_15_["ok?"]
    local err_3f = _let_15_["err?"]
    local function _17_(_241, _242)
      local _18_, _19_ = run(_242)
      if ((nil ~= _18_) and (nil ~= _19_)) then
        local action = _18_
        local value = _19_
        return action, {_242, value}
      elseif true then
        local _ = _18_
        return error("workflow.run did not return 2 values")
      else
        return nil
      end
    end
    local _let_16_ = enum["group-by"](_17_, scheduler.active)
    local halted = _let_16_["halt"]
    local continued = _let_16_["cont"]
    local _
    local function _23_(_0, _21_)
      local _arg_22_ = _21_
      local wf = _arg_22_[1]
      local _result = _arg_22_[2]
      return wf
    end
    scheduler["active"] = enum.map(_23_, (continued or {}))
    _ = nil
    local _0
    local function _24_(_241, _242)
      local function _27_(_1, _25_)
        local _arg_26_ = _25_
        local wf = _arg_26_[1]
        local result0 = _arg_26_[2]
        return broadcast(wf, result0)
      end
      return enum.map(_27_, _242)
    end
...
local function _33_(...)
  ...
  if (nil ~= _35_) then
    local f_74_auto = _35_
    return f_74_auto(...)
  elseif (_35_ == nil) then
    local view_77_auto
    do
      local _36_, _37_ = pcall(require, "fennel")
      if ((_36_ == true) and ((_G.type(_37_) == "table") and (nil ~= (_37_).view))) then
        local view_77_auto0 = (_37_).view
        view_77_auto = view_77_auto0
      elseif ((_36_ == false) and true) then
        local __75_auto = _37_
...
local function _40_()
  local _41_
  do
    table.insert((__fn_2a_new_dispatch).help, "(where {})")
    local function _42_(...)
      if (0 == select("#", ...)) then
        local _43_ = {...}
        local function _44_(...)
    ...
    table.insert((__fn_2a_new_dispatch).bodies, _42_)
    _41_ = new
  end
  local function _48_()
