...
local function sha_3f(sha)
  return (string_3f(sha) and (40 == #string.match(sha, "^(%x+)$")))
end
local function version_3f(v)
  return (string_3f(v) and ((nil ~= string.match(v, "^(%d+)$")) or (nil ~= string.match(v, "^(%d+%.%d+)$")) or (nil ~= string.match(v, "^(%d+%.%d+%.%d+)$"))))
end
local function version_spec_3f(v)
  return (string_3f(v) and (nil ~= string.match(v, "^[%^~><=]+%s?%d+%.%d+%.%d+$")))
end
