...
local function _1_(...)
  ...
  if ((nil ~= _2_) and _3_(...)) then
    local path_3_auto = _2_
    if string.find(full_mod_path_2_auto, "ruin") then
      local _4_ = string.match(full_mod_path_2_auto, ("(.+%.)" .. "ruin"))
      if (_4_ == nil) then
        return ""
      elseif (nil ~= _4_) then
      ...
      return error(string.format("relative-root: no match in &from %q for %q", full_mod_path_2_auto, "ruin"))
    end
  elseif (_2_ == nil) then
    return ""
  else
...
local function lazyload(t, k)
  local function _8_()
    local _9_ = aliases[k]
    if (nil ~= _9_) then
      local _10_ = require(_9_)
      if (nil ~= _10_) then
