...
local function _1_(...)
  ...
  if ((nil ~= _2_) and _3_(...)) then
    local path_3_auto = _2_
    if string.find(full_mod_path_2_auto, "monad") then
      local _4_ = string.match(full_mod_path_2_auto, ("(.+%.)" .. "monad"))
      if (_4_ == nil) then
        return ""
      elseif (nil ~= _4_) then
      ...
      return error(string.format("relative-root: no match in &from %q for %q", full_mod_path_2_auto, "monad"))
    end
  elseif (_2_ == nil) then
    return ""
  else
...
local function m__3e(monad_t, ival, ...)
  ...
  local function _8_(...)
    ...
    for _, f in ipairs({...}) do
      val = monad_t.bind(val, f)
    end
    return val
  end
  return monad_t.result(_8_(...))
end
local identity_m
local function _9_(val, fun)
  return fun(val)
end
local function _10_(val)
  return val
end
identity_m = {bind = _9_, result = _10_}
local maybe_m
do
  local zero_val = nil
  local function _11_(...)
  ...
  maybe_m = {zero = zero_val, plus = _11_, bind = _13_, result = identity_m.result}
end
local state_m
...
state_m = {bind = bind, result = result, set = _17_, get = _19_}
