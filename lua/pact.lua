 ...
 do end (seen_plugins)[plugin_id] = nil else end
 local arg_v = {...}
 local arg_c = select("#", ...) local real_fn
 local function _5_() local mod = require("pact.plugin")
 local f = mod[name]
 return f(unpack(arg_v, 1, arg_c)) end real_fn = _5_
 seen_plugins[plugin_id] = true
 return table.insert(plugin_proxies, real_fn) end return _3_ end
 ...
 error("pact.nvim requires nvim-0.8 or later") else end
 local opts0 = (opts or {})
 do opts0["concurrency-limit"] = (opts0["concurrency-limit"] or opts0.concurrency_limit) end local e_str = "must provide both win and buf or neither"
 ...
 return ui.attach(win, buf, plugins, opts0) end
